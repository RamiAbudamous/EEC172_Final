...
int colors[] = {WHITE, BLUE , GREEN, CYAN , RED  , MAGENTA, YELLOW, 0x8888};
...
void testHorizontalLines() {
   fillScreen(BLACK);
   unsigned int color = 0x1FFF;
   unsigned int y;
   unsigned int i;
   for (y=0; y < height()-1; y+=16) {
        drawFastHLine(0, y, width()-1, colors[i++]);
   }
   delay(100);
}
...
void testVerticalLines() {
    ...
    for (x=0; x < width()-1; x+=16) {
        color += 0x1FFF;
        drawFastVLine(x, 0, height()-1, colors[i++]);
    }
    delay(100);
}
...
void printHelloWorld() {
    fillScreen(BLACK);
    unsigned char helloWorldChars[] = {0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x20, 0x77, 0x6F, 0x72, 0x6C, 0x64, 0x21};
    int i = 0;
    int j = 0;
    int k = 0;

    for(k=0; k<sizeof(helloWorldChars); k++)
    {
        drawChar(i, j, helloWorldChars[k], WHITE, BLACK, 1);
        i += 6;
        if(i >= 123){
            i = 0;
            j += 8;
